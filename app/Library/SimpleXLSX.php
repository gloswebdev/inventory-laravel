...
//      $this->error( 'Invalid cell index ' . $cell );
    ...
    public function readRowsEx($worksheetIndex = 0, $limit = 0)
    {
        if (!$this->rowsExReader) {
            ...
            // Removed requirement for basic usage
    ...
    // Additional helper function `_strlen` was called in original but might be missing, 
    // it was just a local call to `self::strlen` mostly or `strlen` wrapper.
    // I noticed `_strlen` usage in line 349: $this->_strlen($vZ).
    // I will add it as an alias to standard strlen or fix the call site if I find it.
    // Wait, line 349 in original has `$this->_strlen($vZ)`. 
    // I need to implement `_strlen` or change it to `self::strlen`.
    // Looking at the code I read, there is `public static function strlen`, so I should use `self::strlen`.
    // I will implement `_strlen` as a helper just in case.
