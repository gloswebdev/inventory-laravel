...
class SyncMssqlToCloudCommand extends Command
{
    ...
    protected $signature = 'mssql:sync-to-cloud 
                            ...
                            {--truncate : Truncate and perform full sync}
     ...
     * @var string
     */
    protected $description = 'Fetch sales records from local MS SQL Server (via Tailscale) and push to Cloud website MySQL database';
    ...
    public function handle()
    {
        ...
        // 1. Check MS SQL Connection
        try {
            $this->comment("Connecting to MS SQL Server at 100.108.74.58...");
            $res = DB::connection('sqlsrv')->select('SELECT DB_NAME() AS db, COUNT(*) as total FROM Sl_Txn20252026 TXN INNER JOIN Sl_Head20252026 HD ON TXN.vouch_code = HD.vouch_code AND HD.Deleted = 0');
            $totalCount = $res[0]->total ?? 0;
            $this->info("✅ Connected to {$res[0]->db}! Total records in MS SQL for 2025-2026: {$totalCount}\n");
        } catch (\Exception $e) {
            $this->error("❌ Cannot connect to MS SQL Server: " . $e->getMessage());
            return 1;
        }
        ...
        INNER JOIN      
            Sl_Head20252026 AS HD ON TXN.vouch_code = HD.vouch_code AND HD.Deleted = 0 
        LEFT JOIN      
        ...
        $this->info("Fetching records from MS SQL Server...");
        $rows = DB::connection('sqlsrv')->select($query);
        $rowCount = count($rows);
        $fetchTime = round(microtime(true) - $startTime, 2);
        $this->info("Fetched {$rowCount} records in {$fetchTime}s.\n");

        if ($rowCount === 0) {
            $this->warn("No records returned from MS SQL.");
            return 0;
        }
        ...
        foreach ($chunks as $index => $chunk) {
            ...
            $payload = [
                'token'    => $token,
                'truncate' => $isFirstChunk ? true : false,
        ...
        $this->info("• Total Records Pushed: {$pushedCount}");
        $this->info("• Total Duration      : {$totalDuration}s");
        $this->info("• Cloud Endpoint      : {$endpoint}");
        $this->info("=================================================\n");
