...
<div class="space-y-8 pb-10" x-data="usersApp()">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-3xl font-900 text-slate-800 tracking-tighter">User Manager</h2>
    ...
    <div class="space-y-4">
        ...
        <div class="glass-premium p-6 rounded-[2.5rem] border border-white/80 space-y-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h4 class="text-sm font-bold text-slate-800 tracking-tight">{{ $user->name }}</h4>
                        <div class="flex items-center gap-2 mt-1">
                            <span class="px-2 py-0.5 bg-slate-50 text-[8px] font-black uppercase tracking-widest text-slate-400 rounded-lg">@ {{ $user->username }}</span>
    ...
    <div x-show="showAddUser" x-cloak class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-lg bg-white rounded-[3rem] p-8 space-y-6 shadow-2xl animate-slide-up overflow-y-auto max-h-[90vh]">
            <div class="flex items-center justify-between">
                <h3 class="text-2xl font-900 text-slate-800 tracking-tighter">New Access</h3>
                <button @click="showAddUser = false" class="w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <div class="space-y-4">
                <input type="text" x-model="form.name" placeholder="Full Name" class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-sm font-bold text-slate-700">
                <input type="text" x-model="form.username" placeholder="Username" class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-sm font-bold text-slate-700">
                <div class="grid grid-cols-2 gap-4">
                    <select x-model="form.role" class="bg-slate-50 border-none rounded-2xl py-4 px-6 text-sm font-bold text-slate-700">
                        <option value="user">Standard User</option>
                        <option value="admin">Administrator</option>
                    </select>
                    <select x-model="form.interface_type" class="bg-slate-50 border-none rounded-2xl py-4 px-6 text-sm font-bold text-slate-700">
                        <option value="mobile">Mobile View</option>
                        <option value="desktop">Desktop View</option>
                    </select>
                </div>
                <input type="password" x-model="form.password" placeholder="Password (Min 4 chars)" class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-sm font-bold text-slate-700">
            </div>
    ...
    <div x-show="showEdit" x-cloak class="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-lg bg-white rounded-[3rem] p-8 space-y-8 shadow-2xl animate-slide-up overflow-y-auto max-h-[90vh]">
            <div class="flex items-center justify-between">
                <div>
                    <h3 class="text-2xl font-900 text-slate-800 tracking-tighter" x-text="currentUser?.name"></h3>
                    <p class="text-[9px] font-black uppercase tracking-[0.2em] text-indigo-500">Security Configuration</p>
                </div>
                <button @click="showEdit = false" class="w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            ...
            <div class="space-y-8">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <label class="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border-2 border-transparent transition-all" :class="permForm.branches.includes({{ $branch->id }}) ? 'border-indigo-500 bg-white' : ''">
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="space-y-2">
                            ...
                            <label class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl active:bg-slate-100 transition-all">
