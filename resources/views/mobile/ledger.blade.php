...
<div class="space-y-8 pb-10" x-data="ledgerApp()">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-3xl font-900 text-slate-800 tracking-tighter">Ledger</h2>
            <p class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 mt-1">Movement Analytics</p>
        </div>
        <button @click="showFilters = !showFilters" :class="showFilters ? 'bg-indigo-50 text-indigo-500' : 'bg-white text-slate-400'" class="w-12 h-12 rounded-2xl flex items-center justify-center border border-slate-100 shadow-sm transition-all active:scale-90">
            <i class="fas fa-search-plus text-xs"></i>
        </button>
    </div>

    <!-- Quick Search / Filters -->
    <div x-show="showFilters" x-cloak x-transition class="glass-premium p-6 rounded-[2.5rem] border border-white/80 space-y-6">
        <div class="space-y-2">
            <label class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-3">Product</label>
            <select x-model="filters.product_id" @change="applyFilters" class="w-full bg-slate-50 border-none rounded-2xl py-4 px-6 text-xs font-bold text-slate-700">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
                ...
                <input type="date" x-model="filters.from_date" @change="applyFilters" class="w-full bg-slate-50 border-none rounded-2xl py-4 px-5 text-[10px] font-black text-slate-700">
            </div>
            <div class="space-y-2">
                <label class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] ml-3">To</label>
                <input type="date" x-model="filters.to_date" @change="applyFilters" class="w-full bg-slate-50 border-none rounded-2xl py-4 px-5 text-[10px] font-black text-slate-700">
            </div>
        </div>
    </div>
    ...
    <div class="space-y-4">
        ...
        @php
            ...
            ][$item->transaction_type] ?? ['icon' => 'fa-arrow-right-arrow-left', 'color' => 'text-slate-500', 'bg' => 'bg-slate-50', 'label' => 'Transaction'];
        @endphp
        
        <div class="glass-premium p-5 rounded-[2.2rem] border border-white/80 flex items-center justify-between group active:scale-[0.98] transition-all relative overflow-hidden">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 {{ $typeConfig['bg'] }} {{ $typeConfig['color'] }} rounded-2xl flex items-center justify-center border border-white/50 shadow-sm">
                    <i class="fas {{ $typeConfig['icon'] }} text-xs"></i>
                </div>
                <div class="overflow-hidden">
                    <div class="text-[11px] font-900 text-slate-800 tracking-tighter truncate max-w-[140px] uppercase italic">
